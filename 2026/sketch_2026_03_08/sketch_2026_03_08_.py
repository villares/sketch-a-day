...
grid = tuple(product((-3, -1, 1, 3), repeat=2))
seeds = set()
s_combos = []

def setup():
    global all_tris
    py5.size((W + M) * 32 + M, (W + M) * 16 + M)
    py5.color_mode(py5.CMAP, py5.mpl_cmaps.VIRIDIS, 36)
...
#         grow_seeds()
#         print(len(seeds))
    s_combos[:] = (Combo([tri]) for tri in all_tris)
    # sorting for display
    s_combos.sort(

...
"""
 " " (space bar) to create many balls
 "w" to drag the mouse and create a static segment (wall)
 "b" to drag the mouse and create a box
 "k" to drag the mouse and create a kinematic box
 "p" to drag the mouse and create a polygon
 <delete> or <backspace> to remove objects under the mouse
 "c" to remove dynamic objects
 "d" to save state, and "l" to restore saved state.
...
def key_pressed():
    ...
    if py5.key in (py5.BACKSPACE, py5.DELETE):
        ...
        py5.save_pickle(sim, 'sim.data')
        print('saved. ') 
    elif py5.key == "l":
        try:
           sim = py5.load_pickle('sim.data') 

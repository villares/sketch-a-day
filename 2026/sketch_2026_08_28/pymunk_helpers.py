"""
Alexandre B A Villares (2026) 
https://github.com/villares/villares/blob/main/pymunk_helpers.py
v2026-08-28
...
class Simulation:
    ...
    def __len__(self):
        return len(self.living_set)

    def __getstate__(self):
        """Shed the non-picklable py5 sketch reference before pickling."""
        state = self.__dict__.copy()
        state["current_sketch"] = None
        return state

    def __setstate__(self, state):
        """Restore state and reattach a fresh sketch reference on unpickling."""
        self.__dict__.update(state)
        self.current_sketch = py5.get_current_sketch()
...
class SimObj:
    ...
    def remove_from_sim(self):
        ...
        # guard against repeated removal
        if self not in living_set or self in for_removal:
            return  # already removed / marked
        # remove all attached constraints first
        ...
        # mark for removal from living registry
...
class Segment(SimObj):
    ...
    def __init__(self, xa, ya, xb, yb,
                 radius=2,
                 fill_color=None,  # will be used as line stroke color
                 kinematic=False,
                 static=False,
                 simulation=None):
        ...
        self.stroke_color = 0 if fill_color is None else int(fill_color)
...
class Ball(SimObj):
    ...
    def __init__(self, x, y, diameter, fill_color=None, simulation=None):
        ...
        radius = diameter / 2
        self.fill_color = 255 if fill_color is None else int(fill_color)
...
class Poly(SimObj):
    ...
    def __getstate__(self):
        """Shed the non-picklable Py5Shape; draw() recreates it on demand."""
        state = self.__dict__.copy()
        state["py5shape"] = None
        return state
...
class Box(SimObj):
    ...
    def __init__(self, x, y, w, h, fill_color=None, kinematic=False, simulation=None):
        ...
        if fill_color is None:
            self.fill_color = (
                int(py5.color(100, 0, 0)) if kinematic
                else int(py5.color(0, 100, 0))
            )
        else:
...
class PivotJoint(Constraint):
    ...
    def __init__(self, obj_a, obj_b, anchor=None, radius=4, color=None, simulation=None):
        ...
        self.radius = radius
        self.color = int(py5.color(0, 128, 0)) if color is None else int(color)
...
class Spring(Constraint):
    ...
    def __init__(self, obj_a, obj_b, anchor_a=None, anchor_b=None,
        ...
        self.color = int(py5.color(0, 100, 0)) if color is None else int(color)
        self.body_a = obj_a.body if isinstance(obj_a, SimObj) else obj_a
        self.body_b = obj_b.body if isinstance(obj_b, SimObj) else obj_b

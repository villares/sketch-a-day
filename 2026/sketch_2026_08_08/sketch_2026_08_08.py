...
def draw(): 
    ...
    if p == 1:
        ...
        grid_a =  np.array([(W + i * W, 150 + W + j * W) for i, j in grid])
        grid_b =  np.array([(300 + W + i * W, 150 + W + j * W) for i, j in grid])
        xy_grid = np.array([(W + i * W, W + j * W) for i, j in grid])
        pos_a = py5.lerp(xy_grid, grid_a, amt)
        pos_b = py5.lerp(xy_grid, grid_b, amt)

        py5.text_size(W/3)
        for (xa, ya), (xb, yb), (i,j) in zip(pos_a, pos_b, grid):
            py5.square(xa, ya, W)
            py5.text(f'{i}   ', xa, ya)
            py5.square(xb, yb, W)
            py5.text(f'   {j}', xb, yb)

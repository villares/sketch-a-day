...
def quarter_poly(x, y, radius, quadrant, num_points=6):
    poly_arc(x, y, radius, ROTATION[quadrant], py5.HALF_PI, num_points)
    
def half_poly(x, y, radius, quadrant, num_points=6):
...
#             a += angle
    with py5.begin_shape(): 
        for i in range(num_points + 1):
            sx = x + py5.cos(start_ang + i * angle) * radius
            sy = y + py5.sin(start_ang + i * angle) * radius
            py5.vertex(sx, sy)    

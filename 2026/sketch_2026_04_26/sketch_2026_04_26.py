...
def setup():
    py5.size(500, 500)
    py5.color_mode(py5.CMAP, 'viridis', 500 * 100)
...
def draw():
    ...
    for i, sd in enumerate(star_data):
        shp = star(**sd)
        py5.no_stroke()
        inside = py5.dist(sd['x'], sd['y'], mx, my) < max(sd['ra'], sd['rb']) \
                 or shp.contains(shapely.Point(mx, my)) 
        if inside:
            under_mouse = i
            py5.no_fill()
            py5.stroke('red')
            py5.circle(sd['x'], sd['y'],  max(sd['ra'], sd['rb']) * 2)
        py5.fill(shp.area)
        py5.shape(shp)

"""
This sketch was made during an activity at Sesc Av. Paulista,
based on code by Nick McIntyre <https://mcintyre.io/> discussed at
https://discourse.processing.org/t/fluid-fun-with-py5-and-numpy/47943
"""
import py5
import numpy as np

# This is missing the `from py5_tools import gif_animation` code
# I used to save the GIF output, because I forgot to save it :(
...
def draw():
    ...
    # Add sources.
    u_prev[my - 1 : my + 1, mx - 1 : mx + 1] = np.random.uniform(-10, 10)
    v_prev[my - 1 : my + 1, mx - 1 : mx + 1] = np.random.uniform(-10, 10)
    dens_prev[my - 1 : my + 1, mx - 1 : mx + 1] = 100 #np.random.uniform(100)
    ...
    D = normalize(dens[1:-1, 1:-1]) * 255
    U = u[1:-1, 1:-1] 
    V = v[1:-1, 1:-1] 
    
    U_ALTA = (U > 0).astype(int) * 255
    V_ALTA = (V > 0).astype(int) * 255
    
    # Display the RGB array.
    py5.load_np_pixels()
    py5.np_pixels[:, :, 1] = V_ALTA * D # red channel
    py5.np_pixels[:, :, 2] = U_ALTA * D # green channel
    py5.np_pixels[:, :, 3] = D # blue channel
#    py5.set_np_pixels(D, 'L') # all

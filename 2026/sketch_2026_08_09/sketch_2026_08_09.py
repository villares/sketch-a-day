...
def draw():
    ...
    for c in Cell.grid.values():
        c.update(mouse_x - width / 2, mouse_y - height / 2)
    for c in Cell.grid.values():
        c.plot(mode)
...
def key_pressed():
    ...
    if key == "s" or key == "S":
        save_frame("_#######.png")
    if key != CODED and key in "01245679":
        mode = int(key)
    if key == "-":
        mode = -1
    if key == " ":
...
class Cell():
    ...
    def calculate_pos(self):
        ...
        self.pos = Py5Vector(self.border + self.size_ / 2 + i * self.size_ - width / 2,
    ...
    def draw_mode(self):
        ...
        with push_matrix():
            ...
            for i in range(Cell.step_start,
                           ...
                           Cell.step):  # (-28, 29, 7):
                #translate(0, 0, (a + i))
                stroke(200 + i * Cell.step) #16 + i * 8, 255, 255)

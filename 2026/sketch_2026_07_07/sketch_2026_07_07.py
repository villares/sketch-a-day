...
def setup():
    global out, star_infos
    py5.size(1000, 900)
    n = len(grid_points)
    w = np.linspace(20, 40, n)
    h = np.linspace(80, 0, n)
    angs = np.linspace(0, py5.TAU, n)
    star_infos = list(zip(grid_points, w, h, angs))
    animated_gif('out.gif', frame_numbers=range(1, 181, 5), duration=0.27)
...
def draw_star(star_info):
    ...
    py5.fill((h * 10 + f / 180 * 255) % 255, abs(pos[1] / 2), b)
    with py5.push_matrix():
        py5.translate(*pos)
        py5.rotate(py5.radians(f * 2))

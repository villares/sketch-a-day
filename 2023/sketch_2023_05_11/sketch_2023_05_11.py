...
def init_segments():
    Segment.segments = []
    seg = Segment((width / 2, width / 2))
        
    while len(Segment.segments) < num_segs:
        anchor_seg = random_choice(Segment.segments)
        if len(anchor_seg.linked) < 2:
            new_seg = Segment(link=(anchor_seg, anchor_pos))
            anchor_seg.linked.append(new_seg)
            new_seg.length = anchor_seg.length * 1.1

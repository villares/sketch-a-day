...
nice _sliders_list based on Peter Farell's _sliders_list htts://twitter.com/hackingmath
https://github.com/hackingmath/python-_sliders_list http://farrellpolymath.com/
...
2023-07-15 Added KEYBOARD_STEP constant and fixed keyboard crash when using Arduino
"""
...
class InputInterface:
    ...
    def __init__(self, port=None, keyboard_step=2):
        Slider.KEYBOARD_STEP = keyboard_step
        ...
        if self.source is None:
            ...
            self._sliders_list = {1: A, 2: B, 3: C, 4: D}

    def analog_read(self, pin):
        if self.source is not None:
            return self.source.analog_read(pin)
        else:
            return self._sliders_list[pin].val

    def update(self, display=True):
        if self.source is None:
            for pin, slider in self._sliders_list.items():
                slider.update(display=display)
    ...
    def key_pressed(self):
        if self.source is None:
            ...
            if k == 'a':
                self._sliders_list[1].down = True
            if k == 'd':
                self._sliders_list[1].up = True
            if k == 's':
                self._sliders_list[2].down = True
            if k == 'w':
                self._sliders_list[2].up = True
            if kc == py5.LEFT:
                self._sliders_list[3].down = True
            if kc == py5.RIGHT:
                self._sliders_list[3].up = True
            if kc == py5.DOWN:
                self._sliders_list[4].down = True
            if kc == py5.UP:
                self._sliders_list[4].up = True
    ...
    def key_released(self):
        if self.source is None:
            ...
            k, kc = sketch.key, sketch.key_code
            if k == 'a':
                self._sliders_list[1].down = False
            if k == 'd':
                self._sliders_list[1].up = False
            if k == 's':
                self._sliders_list[2].down = False
            if k == 'w':
                self._sliders_list[2].up = False
            if kc == py5.LEFT:
                self._sliders_list[3].down = False
            if kc == py5.RIGHT:
                self._sliders_list[3].up = False
            if kc == py5.DOWN:
                self._sliders_list[4].down = False
            if kc == py5.UP:

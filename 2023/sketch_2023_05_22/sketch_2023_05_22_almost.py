...
def update_grid():
    # flow sideways
    water_empty_right = (grid == WATER) & (np.roll(grid, -1, axis=1) == VOID) & (np.roll(grid, 1, axis=1) != VOID)
    water_empty_left = (grid == WATER) & (np.roll(grid, 1, axis=1) == VOID) & (np.roll(grid, -1, axis=1) != VOID)
    void_water_left = (grid == VOID) & (np.roll(grid, 1, axis=1) == WATER) & (np.roll(grid, 2, axis=1) != VOID)
    
    grid[water_empty_right] = SAND
    grid[void_water_left] = CONCRETE
#     grid[empty_left] = VOID
#     grid[np.roll(empty_left, -1, axis=1)] = WATER        

...
def setup():
    ...
    n = 8 # numero de linhas e colunas da grade de pontos
    w = width / n
    # construir pontos
    calcular_pontos()
    # construir celulas
...
#             y = y + random(-w/8, w/8)
            ...
            for p, k in celulas.values():
                if p_contains(p, x, y):
                    ...
                    break
                region = 0    
            c = 128 if region == 0 else color((region * 32) % 255, 200, 200)
            fill(c)
            with push_matrix():
                translate(0, 0, region * 10)
...
def calcular_celulas():
    ...
    k = 0
    grid = list(product(range(n-1), repeat=2))
    shuffle(grid)
    for i, j in grid:
        cps = (# cell points
            (i, j),

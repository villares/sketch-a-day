...
bldg_color = '#008888'
highlight = '#FF0000'
point = ox.geocode('Museu Lasar Segall, São Paulo, Brasil')
dist = 5000
...
museus = m_fp['name'].str.contains('Museu', case=False, na=False)
...
def setup():
    ...
    drawing_elements[:] = [
     ...
     (paulista.affine_transform(matrix),
          highlight, None),
     (m_fp[museus].geometry.affine_transform(matrix),
          None, highlight),
     ]

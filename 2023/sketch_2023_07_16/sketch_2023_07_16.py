...
onda_x = [(1 + cos(radians(a))) / 2 for a in range(360)]
onda_y = [(1 + sin(radians(a))) / 2 for a in range(360)]
...
def draw():
    ...
    for x in range(spac_size // 2, width, spac_size):  # um x p/ cada coluna
        for y in range(spac_size // 2, width, spac_size):  # um y p/ cada linha
            ...
            offsetX = rand_posi * random(-1, 1) * onda_x[int(frame_count * v) % 360]
            offsetY = rand_posi * random(-1, 1) * onda_y[int(frame_count * v) % 360]

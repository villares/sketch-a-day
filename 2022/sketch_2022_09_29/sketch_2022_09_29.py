...
def draw():
    ...
    if 145 > frame_count >= 100:
        save_frame(f'{frame_count - 100:02}.png')
        print(frame_count)
...
def grow(unvisited_nodes):
    ...
    for i, j, c in unvisited_nodes:
        if not visible(i, j):
            continue
        _, _, gen, cc = nodes.get((i, j, c % 2), (0, 0, 0, k := k + 1))        
        random.seed(c + gen // 5)
        nbs = EVN_NBS if i % 2 == 0 else ODD_NBS
        xnbs = random.sample(nbs, 3)
        for ni, nj in xnbs:
            ini, jnj= i + ni, j + nj
            if visible(ini, jnj) and (ini, jnj, c % 2) not in nodes:
                nodes[(ini, jnj, c % 2)] = (i, j, gen + 1, cc)
                yield (ini, jnj, c)

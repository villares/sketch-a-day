...
pontos = []
starting_points = [(420 + 150 * cos(i * tau / 4), 420 + 150 * sin(i * tau / 4))
                    for i in range(4)]
...
#             py5.curve_vertex(*pts[1])
        ...
        pts[:] = processar(pts)
    if py5.frame_count != 360 / 5:
        pass #py5.save_frame('####.png')
    else:
        py5.exit_sketch()
...
def processar(pts):
    ...
    for a, b in zip(src_pts[:-1], src_pts[1:]):
        va = py5.Py5Vector(a)
        vb = py5.Py5Vector(b)
        vd = (va - vb).rotate(py5.HALF_PI) * sin(radians(py5.frame_count * 5))  / 2
        mp = (va + vb) / 2
        yield a
        yield tuple(mp + vd)

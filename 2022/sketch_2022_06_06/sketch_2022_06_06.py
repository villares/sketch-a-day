...
def build_softpoly(ex, radius, mass, poly):
...
def create_link(me, other, ex):
                link = pm.PinJoint(me, other, (0, 0), (0, 0),)
                       # pm.PinJoint(me, other, (0, 0), (0, 0),ex, stiffness, damping)
                #link = pm.PinJoint(me, other, (0, 0), (0, 0))
                space.add(link)
...
def mouse_clicked():
    if py5.mouse_button == py5.RIGHT:
        build_softbody(py5.mouse_x, py5.mouse_y, cols=8, rows=8, ex=20, radius=5, mass=mass)

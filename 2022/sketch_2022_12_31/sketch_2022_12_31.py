# https://tinyurl.com/feliz-2023-s-e-a
# Código para pyp5js
...
def setup():
    ...
    img.fill(0)
    img.textFont('Mono', int(width / 8))
    img.text(texto1, width / 2, height / 2)
...
def draw():
    ...
    for (x, y), (x0, y0, c, gen) in nodes.items():
        ...
        if message.get((x, y)):
            fill((x + orientation_offset[orientation]) % 255, 255, 255)
        else:
            d  += 0.5
            fill(255)
    ...
    fill(255)
    textFont('Mono')
    text(texto2, 50 - width / 2,  height / 2 - 30)

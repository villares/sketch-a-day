...
def draw():
    background(255, 128, 32)  # it's better to leave the background out!
...
def mouse_pressed():
    if len(segments) < seg_limit:
        ...
        if mouse_button == LEFT:
            ...
            stars.append(s_star(mouse_x - mh, mouse_y - mv))
            print(stars)
...
def s_star(x, y):
    return Polygon(star_pts(x, y, 100, 50, 6),
          holes=[star_pts(x, y, 90, 40, 6)])

...
class Poly():
    ...
    def __init__(self, pts, holes=None, closed=True, lw=None):
        self.pts = pts
        self.holes = holes if holes else [[]]
        self.closed = not closed; self.toggle_closed()

    def plot(self):
        id = self.polys.index(self)
        pushStyle()
        strokeJoin(ROUND)
        strokeWeight(self.lw)
        if self.selected_drag == id:
            stroke(200, 0, 0)
        else:
            stroke(0)
        if len(self.pts) >= 2:
            ...
            if self.closed:
                ...
                endShape()
        if self.text_on:
            self.annotate_pts(id, self.pts, color(200, 0, 0), 5)
            for h in self.holes:
                self.annotate_pts(id, h, color(0, 0, 200), 5)
        popStyle()
    ...
    @classmethod
    def setup_grid(cls, cell_size, order, x_offset, y_offset, mode=None):
        cls.cell_size = cell_size
        cls.order = order
        cls.x_offset, cls.y_offset = x_offset, y_offset
        cls.mode = mode or cls.CREATE
    ...
    #     # vai quebrar com pontos sobrepostos
    #     if len(pts) < 3:
    #         return pts

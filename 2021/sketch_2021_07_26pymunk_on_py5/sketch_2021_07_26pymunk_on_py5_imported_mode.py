...
def add_ball(space):
    ...
    body.position = x, -50
    shape_ = pm.Circle(body, radius, (0, 0))
    shape_.friction = 1
    space.add(body, shape_)
    return shape_


def add_L(space):
    """Add a inverted L shape_ with two joints"""
...
def is_ball(obj):
    return isinstance(obj, pm.shapes.Circle)
     
def is_segment(obj):
    return isinstance(obj, pm.shapes.Segment)

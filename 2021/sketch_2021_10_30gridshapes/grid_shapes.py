...
class Shape(object):
    ...
    def raise_order(self, size_limit):
        ...
        for s in self:
            ...
            for adjacent in adjacents:
                new_shape = Shape(list(self) + [adjacent]).translate()
                if Shape.under_limit(new_shape, size_limit):
                    shapes.append(new_shape)
        return shapes

    @staticmethod
    def under_limit(shp, size_limit):
    ...
    @classmethod
    def generate_shapes(cls, order=3):
        grid_size = order * order
        all_shapes = set([cls(((0,0),))])
        current_shapes = all_shapes.copy()
        step = 1
        while step < grid_size:
            step += 1
            next_shapes = set()
            for s in current_shapes:
                next_shapes.update(s.raise_order(size_limit=order))
            current_shapes = next_shapes
            all_shapes.update(next_shapes)
        return all_shapes

...
class Cell():
    ...
    def display(self):
        fill(2 * self.z % 256, 255, 255)
        slab(self.x, self.y, self.z, self.spacing ) #/ 2.0)
    ...
    def display_line(self):    
        for cell in self.links:
            ...
            strokeWeight(3)
            if cell.z > self.z:
                line(self.x, self.y, self.z + SLAB_DEPTH / 2,
                    mid_point_x, mid_point_y, cell.z + SLAB_DEPTH / 2)
            else:
                line(self.x, self.y, self.z + SLAB_DEPTH / 2,
                    mid_point_x, mid_point_y, self.z + SLAB_DEPTH / 2)
    ...
    def update(self):
        if self.current and self.grid_running:
            ...
            if self.unvisited_nbs:
                ...
                if self.kind == 0:
                    rnd_choice.kind = 1# choice((-1, 1))
                    rnd_choice.z = self.z + self.spacing / 4.0 * rnd_choice.kind
                else:
                    rnd_choice.kind = choice((0, 1))

...
SKETCH_NAME, OUTPUT = "sketch_190409a", ".gif"
"""
With glue tabs!
"""
...
cd_i = [box_h + 5, box_h - 5, box_h - 5]  # height of points between d and c
ab_i = [box_h - 5, box_h + 5, box_h + 5]  # height of points between a and b
...
def draw_unfolded():
    ...
    # main outline cut
    num_i = len(cd_i)
    cd_pts = tuple([(box_w + box_d * (i + 1) / (num_i + 1), -cd_i[i])
                    for i in range(num_i)])
    ab_pts = tuple([(box_w * 2 + box_d + box_d * (i + 1) / (num_i + 1), -ab_i[i])
                    for i in range(num_i)])
    main_outline = (ch_2d,) + cd_pts + (dh_2d, ah_2d) + ab_pts + (
        # (box_w + box_d / 3, -dc_i[0]),
        # (box_w + box_d * 2 / 3, -dc_i[1]),
        (box_w * 2 + box_d * 2, -bh),
        (box_w * 2 + box_d * 2, 0),
        c0_2d)
...
def draw_3d():
    ...
    num_i = len(cd_i)
    cd_pts = ((box_w, box_d, dh),
              ) + tuple([(box_w, box_d - box_d * (i + 1) / (num_i + 1), cd_i[::-1][i])
                         for i in range(num_i)]) + ((box_w, 0, ch),)
    ab_pts = ((0, 0, bh),
              ) + tuple([(0, box_d * (i + 1) / (num_i + 1), ab_i[i])
                         for i in range(num_i)]) + ((0, box_d, ah),)
    # face 0
    poly_draw(((0, 0, bh),
               (box_w, 0, ch),
               (box_w, 0, 0),
    ...
    for i in range(1, len(ab_pts)):
        p = i - 1
        triangulated_face(cd_pts[::-1][p], ab_pts[p], ab_pts[i], cd_pts[::-1][i])

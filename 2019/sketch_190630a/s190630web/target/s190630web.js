// Transcrypt'ed from Python, 2019-07-01 23:51:16
...
var __name__ = 's190630web';
export var border = 100;
export var two_triangle_combos = list ([]);
...
export var setup = function () {
	...
	var points = combinations (grid, 3);
	var triangles = list ([]);
	for (var p of points) {
		var area = (p [1] [0] * (p [2] [1] - p [0] [1]) + p [2] [0] * (p [0] [1] - p [1] [1])) + p [0] [0] * (p [1] [1] - p [2] [1]);
		if (area != 0) {
			triangles.append (p);
		}
	}
	print ('Number of possible triangles: {}'.format (len (triangles)));
	two_triangle_combos = list (combinations (triangles, 2));
	print ('Number of 2-triangle combinations: {}'.format (len (two_triangle_combos)));
};
export var draw = function () {
	...
	noFill ();
	var i = __mod__ (Math.floor (frameCount / 90), len (two_triangle_combos));
...
export var draw_combo = function (combo) {
	...
	noFill ();
	var siz = space * 0.5;
	var triangles = tuple ([combo [0], lerp_poly (combo [0], combo [1], 0.33), lerp_poly (combo [0], combo [1], 0.66), combo [1]]);
	for (var [i, t] of enumerate (triangles)) {
...
//# sourceMappingURL=s190630web.map
...
SKETCH_NAME, OUTPUT = "sketch_190314a", ".gif"
"""
...
def desenho_plot(i, d):
    ...
    for p0 in d:
        translate(0, 0, 1)
        p1, p2 = p0.points_to  # se estiver apontando para alguém
        with pushMatrix():
           # for i in range(LEVELS):
            # , 255, 255) #164 + i * 8,, 255, 255, 64)
            fill(128 - 128 * cos(i  / 10.), 100)
            stroke(128 - 128 * sin(i  / 10.0))
             # translate(0, 0, 48)
            ...
            var_bar(p1.x, p1.y,
                    p2.x, p2.y,
                    p1.radius_size + p1.radius_size* cos(i  / 10.0), 
                    p2.radius_size + p2.radius_size* sin(i  / 10.0))
...
def novo_desenho(desenho):
    ...
    for X in X_LIST:
        for Y in Y_LIST:
            desenho.append(Node(X, Y, rnd.choice([20, 40, 60]), []))

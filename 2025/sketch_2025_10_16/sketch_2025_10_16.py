...
circles = [Point(py5.random_int(-100, 100), py5.random_int(-100, 100)).buffer(py5.random(100, 150)) for _ in range(20)]
...
def draw():
    ...
    max_area = max(shp.area for shp in all_circles.geoms)
    py5.translate(250, 250)
    for shp in all_circles.geoms:
        py5.fill(shp.area / max_area * 240, 255, 255)
        py5.shape(shp) 

...
#             pickle.dump(geodata, f)
    ...
    py5.fill(200)
    py5.no_stroke()
    building_types = sorted(set(a for a in buildings.building if str(a) != 'nan'))
    amenity_types = sorted(set(a for a in buildings.amenity if str(a) != 'nan'))
    py5.color_mode(py5.CMAP, 'Oranges', 255)
    b_color = {a: py5.color(i * (255 / len(building_types))) for i, a in enumerate(building_types)}
    py5.color_mode(py5.CMAP, 'viridis', 255)
    a_color = {a: py5.color(i * (255 / len(amenity_types))) for i, a in enumerate(amenity_types)}
    for g, b, a in zip(buildings.geometry, buildings.building, buildings.amenity):
        py5.fill(b_color.get(b, 0))
        py5.stroke(a_color.get(a, 0))
        main_shp.add_child(py5.convert_shape(g))
    # Walk
    edges = geodata['edges']

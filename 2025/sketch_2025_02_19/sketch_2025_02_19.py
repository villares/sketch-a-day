...
def dashed_lines(lines, d=5):
    for line in lines:
        dashed_line(*line, d)

def dashed_line(ax, ay, az, bx, by, bz, d=5):
    a = np.array((ax, ay, az))
    b = np.array((bx, by, bz))
    c = py5.dist(ax, ay, az, bx, by, bz)
    n = int(c / d)
    t = np.linspace((0, 0, 0), (1, 1, 1), n)

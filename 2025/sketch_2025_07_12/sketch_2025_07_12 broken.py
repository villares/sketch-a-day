...
class Maze:
    ...
    def set_player_at_start(self, player):
        player.position = py5.Py5Vector3D(
...
class Block:
    ...
    def update(self):
        player_left = player.position.x - player.dimensions.x / 2
        player_right = player.position.x + player.dimensions.x / 2
        player_top = player.position.y - player.dimensions.y / 2
        player_bottom = player.position.y + player.dimensions.y / 2
        player_front = player.position.z - player.dimensions.z / 2
        player_back = player.position.z + player.dimensions.z / 2
        ...
        if (((box_left < player_left < box_right) or (box_left < player_right < box_right)) and 
            ...
            if x_overlap < y_overlap and x_overlap < z_overlap:
                if box_left_overlap < box_right_overlap:
                    player.position.x = box_left - player.dimensions.x / 2
                else:
                    player.position.x = box_right + player.dimensions.x / 2
            
            elif y_overlap < x_overlap and y_overlap < z_overlap:
                if box_top_overlap < box_bottom_overlap:
                    player.position.y = box_top - player.dimensions.y / 2
                    player.velocity.y = 0
                    player.grounded = True
                else:
                    player.position.y = box_bottom + player.dimensions.y / 2
            
            elif z_overlap < x_overlap and z_overlap < y_overlap:
                if box_front_overlap < box_back_overlap:
                    player.position.z = box_front - player.dimensions.x / 2
                else:
                    player.position.z = box_back + player.dimensions.x / 2
...
class Player():
    ...
    def position(self, pos):
        self.cam.position.x = pos.x
        self.cam.position.y = pos.y
        self.cam.position.z = pos.z
    
    def update(self):
        self.velocity = self.velocity + self.gravity
        self.cam.position.y = self.position.y + self.velocity.y        

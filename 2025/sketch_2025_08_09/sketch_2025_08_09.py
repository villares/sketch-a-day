...
def grow(t):
    ...
    for edge in (((xa, ya), (xb, yb)),
                 ...
                 ((xc, yc), (xa, ya))):
        ea, eb = edge
        a = atan2(eb[1] - ea[1], eb[0] - ea[0]) + pi
        m = midpoint(edge)
        d = py5.dist(*ea, *eb) * 0.87
        p = point_offset(m, d, a - pi / 2)
        if not all_tris.contains(shapely.Point(p)):
            #n = np.array((e0, p, e1), dtype=Triangle)
            n = (ea, p, eb)
            unvisited_tris.append(n)
            all_tris = all_tris.union(shapely.Polygon(n))

...
def setup():
    ...
    print(s.area)
    py5.no_stroke()
    py5.fill(1 - s.area / 80000) # 1
    py5.shape(s)
    for _ in range(500):
        s = shapely.affinity.rotate(s, 1).buffer(-2,) # join_style='mitre')
        py5.fill(1 - s.area / 80000)
        c = shapely.affinity.translate(s, py5.random_int(-250, 250), py5.random_int(-250, 250))
        py5.shape(c)
    print(s.area)

...
def setup():
    ...
    for n in range(2, 5 + 1):
        if Path(f'a4ncombos{n}.pkl').is_file():
            m = py5.millis()
            a4ncombos = py5.load_pickle(f'a4ncombos{n}.pkl')
            a4allcombos.extend(a4ncombos)
            print(f'time to load {n}combos pickle {py5.millis()-m}')
        else: 
            m = py5.millis()
            ncombos = [combo for combo in combinations(tris, n)]
            allcombos.extend(ncombos)
            print(n, len(ncombos))
            py5.save_pickle(ncombos, f'ncombos{n}.pkl')
            a4ncombos = [
                combo for combo in ncombos
                if sum(map(lambda p:p.area, combo)) == 4
                and unary_union(combo).area == 4
            ]
            py5.save_pickle(a4ncombos, f'a4ncombos{n}.pkl')
            a4allcombos.extend(a4ncombos)
...
def draw():
    py5.background(0)
    py5.fill(255, 200)
    N = 15 # rows and columns
    i = 2 # py5.frame_count * N * N
    s = 60
    w = 25
    for r in range(N):
        for c in range(N):

...
grid = [(x, y) for x, y in product(range(-225, 226, 150), repeat=2)
...
def generate():
    ...
    ap = all_shapes.buffer(-5).buffer(5) 
    nap = negative.buffer(-5).buffer(5) 
    ...
    for poly in polys.geoms:
        ...
        malha = trimesh.creation.extrude_polygon(poly, 20 - poly.area / max_area * 15)
        py5.fill(poly.area / max_area * 255)
        result.add_child (py5.convert_shape(malha, min_angle=py5.radians(10)))

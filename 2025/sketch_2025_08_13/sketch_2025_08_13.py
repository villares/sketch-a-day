...
def draw():
    ...
    py5.stroke(0, 150)
    for p in polys:
        py5.fill(p.area * 0.1 % 255, 255, 255, 150)
        py5.shape(p)
...
def key_pressed():
    ...
    if py5.key == ' ':
        ...
        current_unvisited = unvisited_polys[:]
        unvisited_polys.clear() 
        for p in current_unvisited:
            grow(p)
...
def grow(poly):
    ...
    for edge in zip(p, p[1:] + (p[0],)):
        ...
        new_p = shapely.Polygon((ea, pa, pb, eb))
        overlap = all_polys.intersects(new_p.buffer(-1))
        if not overlap and new_p.area > 5:
            unvisited_polys.append(new_p)
            try:

...
def setup():
    global main_shp, geodata, graph, buildings
    py5.size(1000, 1000)
    ...
    if data_path.is_file():
        ...
        graph = ox.graph_from_point(
             se, dist=DIST, network_type='walk',
             )
    ...
    for g, a, lu in zip(buildings.geometry, buildings.amenity, buildings.landuse):
        if lu in ('recreation_ground' , 'grass'):
            py5.fill(0, 100, 0)
        elif not isinstance(lu, float):
            py5.fill(255, 0, 0)
            py5.stroke(128)    

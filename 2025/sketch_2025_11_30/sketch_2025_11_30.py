...
def key_pressed():
    if py5.key == ' ':
        ...
        for x, y in product(range(200, py5.width - 199, 100), repeat=2):
            for _ in range(3):
                particles.append(Particle(x, #+ py5.random(-50, 50),
                                          y #+ py5.random(-50, 50)
                                          ))
...
class Particle:
    ...
    def update(self):
        ...
        self.move()
        self.d = self.d * 0.99
        if self.d < 20 and self.can_split:
            self.can_split = False
            particles.append(Particle(self.pos.x, self.pos.y, 20,
                                      can_split=False))
        elif self.d < 1:
    ...
    def display(self):
        py5.no_stroke()
        py5.fill(py5.remap(self.d, 0, 50, 0, 255), 16)

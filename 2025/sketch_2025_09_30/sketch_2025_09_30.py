...
def setup():
    ...
    for i in range(60):
        if i % 5 == 0:
            x, y = py5.random_int(-250, 250), py5.random_int(-250, 250)
        c = shapely.affinity.scale(s, 1 - i / 60, 1 - i / 60)
        c = shapely.affinity.rotate(c, i * 5)
        c = shapely.affinity.translate(
            c, x, y
        )
        py5.fill(1 - c.area / base, 150)
        py5.shape(c - c.buffer(-5))
        print(c.area)

...
def draw():
    ...
    for i in range(len(draggable) + 3):
        curveVertex(
            draggable[i % len(draggable)].x,
            draggable[i % len(draggable)].y)
    endShape()
...
def mousePressed():
    global drag
    for i, p in enumerate(draggable):
...
def mouseClicked():
    if not drag:
        ...
        for i in range(len(draggable)):
            ...
            averageX = draggable[i].x + \
                (draggable[nextIndex].x - draggable[i].x) / 2
            averageY = draggable[i].y + \
                (draggable[nextIndex].y - draggable[i].y) / 2
...
def mouseDragged():
    if drag is not None:
        draggable[drag] += PVector(mouseX - pmouseX, mouseY - pmouseY)

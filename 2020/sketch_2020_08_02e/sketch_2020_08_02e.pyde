...
s = 100
b, v, c = {(i % s, i / s)for i in range(s * s)if random(2) > 1}, lambda p: ((p[0] + i, p[1] + j)for i, j in t.product(
    (-1, 0, 1), (-1, 0, 1))if i + j * 2), lambda p: sum((o in b)for o in v(p))
def draw():
    global b
    clear()
    b = {set(p[0], p[1], -1)or p for p in t.chain(*map(v, b))
         if c(p) == 3 ^ (c(p) == 2 and p in b)}

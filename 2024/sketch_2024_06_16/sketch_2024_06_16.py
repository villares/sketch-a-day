...
def setup():
    ...
    poly = shapely.Polygon((v.x, v.y) for i in range(path.get_vertex_count())
...
class DrawableBody(pm.Body):
    def draw(self):
        with py5.push():
            ...
            py5.shape(shp, 0, 0, self.py5_shape.width, self.py5_shape.height)
...
def key_typed():
    global starting_x, flag_delete
    if py5.key == 'd':
        flag_delete = True
    elif py5.key == py5.ENTER:
        starting_x = margin * 2
    else:
        add_triangulated_body(shapely_translate(poly, starting_x, 0))

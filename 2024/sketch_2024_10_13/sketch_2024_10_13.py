...
def start():
    ...
    configurations[:] = sorted(set(configs), key=num_colors_max_area)
...
def num_colors_max_area(config):
    return len(set(r.filled for r in config)) * 1000 + len(config) * 100 + max(r.p.area for r in config) 
...
class ColorArray:
    def __init__(self, array, N, M):        
        _, inverse_indices = np.unique(array, return_inverse=True)
...
class Region:
    ...
    def draw(self, s=None):
        ...
        with py5.push():
            py5.scale(s)
            py5.fill(((self.filled) * COLOR_FACTOR) % 255, 128, 196)

    ...
    for a, b in perms:
        ...
        for c in ((0,0), (0, 1), (1, 1), (1, 0)):
            ...
            if area1 != area0:
                ...
                same.add(Polygon(shp1).buffer(0))
                same.add(Polygon(comp_shp1).buffer(0))
            for d in ((0,0), (0, 1), (1, 1), (1, 0)):
                ...
                if (sp.is_simple and
                    ...
                    same.add(Polygon(shp2).buffer(0))
                    same.add(Polygon(comp_shp2).buffer(0))
                for e in ((0,0), (0, 1), (1, 1), (1, 0)):
                    shp3 = (c, d, e) + edge[a:b+1]
                    sp = Polygon(shp3)
                    area3 = poly_area(shp3)
                    if (sp.is_simple and sp.buffer(0) not in same and
                        area3 != area2 and 
                        area3 != area1 and
                        area3 != area0):
                        comp_shp3 = edge[:a + 1] + (e, d, c) + edge[b:]
                        shapes.append((shp3, comp_shp3))            
                        same.add(Polygon(shp3).buffer(0))
                        same.add(Polygon(comp_shp3).buffer(0))


    print(W * H, len(shapes))

    i = 0
    scale_factor = space * 0.30
        ...
        if i < len(shapes):
            ...
            with py5.push_matrix():
                ...
                with py5.push_matrix():
                    ...
                    j = 0
                    for shp in shps:
                        py5.fill((i + j * 256/2) % 255) #, 255, 255, 100) 

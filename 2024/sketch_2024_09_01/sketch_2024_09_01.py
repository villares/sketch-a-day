...
polys = [
    Point((150, 150)).buffer(100),
    Point((200, 200)).buffer(100),
    Point((300, 200)).buffer(100),
#     Point((300, 300)).buffer(100),
#     Point((300, 350)).buffer(100),
#     Point((350, 300)).buffer(100),
#     Point((200, 300)).buffer(100),
#     Point((250, 350)).buffer(100),
...
def get_regions(polys):
    ...
    unprocessed = polys.copy()
    regions = [unprocessed.pop()]
    while unprocessed:
        a = unprocessed.pop()
        for b
        
def split(a, b):
    inter = intersection(*combo)
    if  not inter.is_empty:
        regions.extend(flatten(inter))
        diff = symmetric_difference(*combo)
        if not diff.is_empty:
            regions.extend(flatten(diff))
